...
void dfs(int v, int start, int p = -1)
{
    used[v] = true;
    cur.pb(v);
    for (size_t i = 0; i < g[v].size(); ++i)
    {
        ...
        if (to == start && to != p)
        {
            ...
            for (auto x: cur)
                tmp.pb(x);
            cerr << tmp << endl;
            cerr << for_ans.count(tmp) << endl << endl;
            bool ok = (for_ans.count(tmp) == 0);
            for (int j = 0; j < tmp.size() && ok; ++j)
            {
                shift(tmp);
                ok = (for_ans.count(tmp) == 0);
            }
            if (ok) 
                for_ans.insert(tmp);
            // cur.erase(v);
